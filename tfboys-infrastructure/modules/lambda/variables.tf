...
# 项目根目录
variable "root_dir" {
  description = "Root directory of the backend project (absolute path to tfboys-backend)"
  type        = string
}
...
# Go 构建命令 (优化版本 - 减少50%文件大小)
variable "build_command" {
  description = "Command to build Go Lambda function"
  type        = string
  default     = "GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -ldflags='-s -w' -o bootstrap main.go"
}
